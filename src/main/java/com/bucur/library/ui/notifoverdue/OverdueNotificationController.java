...
import com.bucur.library.ui.notifoverdue.emailsender.EmailSenderController;
...
public class OverdueNotificationController implements Initializable {
    ...
    private void handleSendNotificationAction(ActionEvent event) {
        ...
        Object controller = LibraryAssistantUtil.loadWindow(getClass().getResource("/fxml/email_sender.fxml"), "Notify Overdue", null);
        if (controller != null) {
            EmailSenderController cont = (EmailSenderController) controller;
            cont.setNotificationRequestData(selectedItems);
            cont.start();
        }
    }
