...
import com.bucur.library.data.model.Book;
import com.bucur.library.data.model.MailServerInfo;
import com.bucur.library.ui.memberlist.MemberListController.Member;
...
public class DataHelper {
    ...
    public static boolean isMemberExists(String id) {
        try {
            String checkStatement = "SELECT COUNT(*) FROM MEMBER WHERE id=?";
    ...
    public static MailServerInfo loadMailServerInfo() {
        try {
            String checkStatement = "SELECT * FROM MAIL_SERVER_INFO";
            PreparedStatement stmt = DatabaseHandler.getInstance().getConnection().prepareStatement(checkStatement);
            ResultSet rs = stmt.executeQuery();
            if (rs.next()) {
